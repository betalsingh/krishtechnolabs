...
(function($){'use strict';var xhr;var active=false;var timer;function create_select2_dropdown(select_el){var args_with_desc={allowClear:btf_localization.allow_clear,minimumResultsForSearch:parseInt(btf_localization.min_search),templateResult:formatResult,templateSelection:formatSelection,dropdownCssClass:':all:',containerCssClass:':all:',syncCssClasses:true}
var args_without_desc={allowClear:btf_localization.allow_clear,dropdownCssClass:':all:',containerCssClass:':all:',syncCssClasses:true,minimumResultsForSearch:parseInt(btf_localization.min_search)}
if(typeof select_el!=='undefined'){if(btf_localization.show_description=='1'){var select2=select_el.select2(args_with_desc);}else{var select2=select_el.select2(args_without_desc);}}else{if(btf_localization.show_description=='1'){var select2=$('.beautiful-taxonomy-filters-select').select2(args_with_desc);}else{var select2=$('.beautiful-taxonomy-filters-select').select2(args_without_desc);}}}
...
var $term=$('<span class="'+term.element.className+'">'+new_term+'</span>');return $term;};function formatSelection(term){if(!term.id||term.text.indexOf(":.:")===-1){return term.text;}
var new_term=term.text;var re=/(:\.:[\s\S]*?:-:)/;var reg_results=re.exec(new_term);new_term=new_term.replace(reg_results[0],'');var $term=$('<span>'+new_term+'</span>');return $term;};function conditional_terms_ajax(el){if(active){xhr.abort();}
