...
The Beautiful Taxonomy Filters plugin is an easy and attractive way to provide your visitors with filtering for your custom post types. With BTF you get a complete solution for adding filtering based on custom taxonomy terms/categories/tags. It will also automatically add rewrite rules for pretty filter URLs. It’s completely automatic, works with or without javascript and is based on the [WordPress Plugin boilerplate](https://github.com/tommcfarlin/WordPress-Plugin-Boilerplate) for a *standardized, organized and object-oriented* codebase. It uses [select2](http://ivaynberg.github.io/select2/) for pretty looking and user friendly dropdowns and will fall back to ordinary select elements if javascript is not supported.
...
* Beautifies the resulting URLs. You won’t see any /post-type/?taxonomy=term. Instead you’ll see /post-type/taxonomy/term.
...
* Comes with a complete extensible filter module for you to put anywhere in your theme.
* Three alternatives for putting the filter modules in your theme:
  * Widgets
  * do_action hooks. 
  * Automagic setting which will automagically place the modules in your archive from thin air. Wizards at work…
* Choose from different styles for the component, a bare minimum style or disable styling and do it all yourself! Just want to tweak a style? Add your custom CSS directly on the settings page.
* Many more settings for fine-tuning the filter modules behavior:
  * Use conditional dropdowns.
...
* Allows for custom GET parameters to be included. Extend the filter your way with a search field, meta value filtering etc. etc. The sky's the limit!
...
The plugin should also load in translations from WordPress automatically.

= Other =
* Based on [WordPress Plugin Boilerplate](https://github.com/tommcfarlin/WordPress-Plugin-Boilerplate)
* Uses [Select2](http://ivaynberg.github.io/select2/) to enhance dropdown interface
...
Why thank you! We don't have proper donate link but if you want to you can send us a giftcard on [fancy](https://fancy.com) and make it out to jontedejong@gmail.com. We will use it to buy cool stuff for the office!
