...
function cs_casepost_func($atts) {  
	...
	if($id) { 
		...
		if($custom_data) {
			...
			$shortdesc = $custom_data['cs_content_shortdes'][0];
	...
	if(!empty($casestudies)) {
		$html .= '<section id="__mainContainer">
			<section class="mainContainer cs_container">
				...
				<ul class="ul_case_studies_listing">';
					foreach($casestudies as $casestudie):
						//$compBanner = wp_get_attachment_link( get_post_meta($casestudie->ID, 'cs_list_image', true), 'medium');
						$compBannerSrc = wp_get_attachment_image_src(get_post_meta($casestudie->ID, 'cs_list_image', true), 'cs-big');
						...
						if(empty($compBanner)):
							...
							if(!empty($cs_list_noimg) && empty($cs_list_svgnoimg)):
								$compNoBannerSrc = wp_get_attachment_image_src($cs_list_noimg, 'cs-big');
							...
							elseif(!empty($cs_list_noimg) && !empty($cs_list_svgnoimg)):
								$compNoBannerSrc = wp_get_attachment_image_src($cs_list_noimg, 'cs-big');
								$compNoBanner = $compNoBannerSrc[0];
								$compSVGBannerSrc = wp_get_attachment_image_src($cs_list_svgnoimg, 'cs-big');
						...
						$desc = get_post_meta($casestudie->ID,'cs_content_shortdes',true);
						$html .= '<li class="item item-'.$class.'"><div class="container">';
							if(!empty($compBanner)):
								$html .= '<div class="image-wrapper wow fadeInLeft animated"><a href="'.$url.'"><img width="'.$width.'" height="'.$height.'" src="'.$compBanner.'" alt="'.$casestudie->post_title.'"></a></div>
									<div class="short-description-wrapper">
									<a href="'.$url.'"><h4 class="wow fadeInRight"><span>'.$casestudie->post_title.'</span></h4></a>';
									if(!empty($desc)):
										$html .= '<aside class="wow fadeInRight animated">'.$desc.'</aside>';
									endif;
								$html .= '<a class="action wow fadeInRight animated" href="'.$url.'">Case Study</a></div>';
							else:
								if(!empty($cs_list_noimg) && empty($cs_list_svgnoimg)):
									$html .= '<a href="'.$url.'">
