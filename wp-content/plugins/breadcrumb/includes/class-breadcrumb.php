...
class breadcrumb
	{
		...
		public function breadcrumb_get_page_childs()
			{
				$this->breadcrumbSeparator = $breadcrumb_separator = $this->breadcrumb_get_option('breadcrumb_separator');
					...
					if (($home->ID) != ($post->ancestors[$i]))
						{
							...
							if($this->get_current_page_depth() > 1) {
								$firstBDCount++;
							}
							if($firstBDCount > 2) {								
								break;
							} else if($firstBDCount == 1) {
								break;
							}
						}
				}
				if($this->get_current_page_depth() < 2) {
		...
		public function breadcrumb_html($themes)
			{
				...
				$html.= '<ul class="leftsection" itemtype="http://schema.org/BreadcrumbList" itemscope="">';		
...
//echo '<pre>'.var_export($parent_cat_links,true).'</pre>';
		...
		public function get_current_page_depth()
		{
			global $wp_query;
